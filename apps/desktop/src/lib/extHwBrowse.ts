/**
 * R20-A：外部作业「桌面模式」内嵌浏览（移动端救急）——官方网页打开通道的执行层。
 *
 * 背景：雨课堂等外部作业的官方网页版未做移动适配，移动端此前点击详情链接
 * 直接丢给系统浏览器（页面小字、来回切换）。本模块把外部作业的「网页打开」
 * 动作统一收口（R20-B2b 起行点击入口在 ./homeworkEntry.ts，本模块是其
 * external-web 分支与原生详情页「浏览器打开」备用出口的共同执行层）：
