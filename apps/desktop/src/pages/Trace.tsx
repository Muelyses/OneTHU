...
export function TracePage(): React.ReactNode {
  ...
  /* ── 定位级联（WGS-84→GCJ-02）：
     ① macOS 原生命令（自建 CoreLocation 桥——官方插件桌面端是返回 (0,0) 的 stub）
     ② 官方插件（仅移动端有真实现；桌面 stub 的 (0,0) 被 null-island 守卫拒收）
     ③ 12h 内上次定位 ④ 清华园中心 ── */
  const locate = useCallback(async () => {
    let got: { lng: number; lat: number; source: "gps" } | null = null;
    // null-island 守卫：非数值或 (0,0)（插件桌面 stub / 无效定位）一律拒收
    const fixGps = (wgsLat: number, wgsLng: number): { lng: number; lat: number; source: "gps" } | null => {
      if (!Number.isFinite(wgsLat) || !Number.isFinite(wgsLng)) return null;
      if (wgsLat === 0 && wgsLng === 0) return null;
      const [lng, lat] = wgs84ToGcj02(wgsLng, wgsLat);
      return { lng, lat, source: "gps" };
    };
    // ① macOS 原生命令（内部 15s 超时，含等待系统授权窗点允许的时间）
    try {
      const r = (await invoke<number[]>("macos_location")) as [number, number, number];
      if (Array.isArray(r) && r.length >= 2) got = fixGps(r[0]!, r[1]!);
    } catch { /* 非 macOS / 命令失败 → 下一路 */ }
    // ② 移动端官方插件
    if (!got) {
      try {
        ...
        if (p) got = fixGps(p.coords.latitude, p.coords.longitude);
      } catch { /* 拒绝授权 / 设备无定位能力 */ }
    }
    if (got) {
    ...
    // ③ 12h 内上次定位（同样过守卫——旧版本可能存过被污染的 (0,0)）
    try {
      const saved = localStorage.getItem(LS_POS);
      if (saved) {
        const p = JSON.parse(saved) as { lng: number; lat: number; at: number };
        const ok = Number.isFinite(p.lng) && Number.isFinite(p.lat) && !(p.lng === 0 && p.lat === 0);
        if (ok && Date.now() - p.at < 12 * 3600_000) {
  ...
  useEffect(() => {
    ...
    // 视野：标注 + 我的位置（仅当定位在学校 ~50km 内才纳入，防异常坐标把视野缩到全球白板）
    if (markersChanged) {
      const pts: L.LatLngExpression[] = markers.map((m) => [m.poi.lat, m.poi.lng]);
      if (origin) {
        const dLat = (origin.lat - CAMPUS_CENTER[0]) * 111; // 纬度 ~111km/度
        const dLng = (origin.lng - CAMPUS_CENTER[1]) * 85; // 北京纬度经度 ~85km/度
        if (Math.hypot(dLat, dLng) < 50) pts.push([origin.lat, origin.lng]);
      }
