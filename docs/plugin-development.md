...
| 一个原子占满 | 列表 | 课程 / 作业 / 洗衣机 / 教室等显示其详情，**高度越大行数越多**（最多 5 行）；教室显示「本节空闲 / 占用 + 今日空闲节次」，洗衣机显示「使用中 · 剩 N 分钟 + 本楼空闲台数」（捷利 / 海乐生活 / 小兰智慧三家数据源）。实时值由应用在计算内容前获取，获取失败时该行不写入 |
...
删除小组件**：添加与删除由用户在桌面上完成，插件只能修改已存在小组件的内容。
