...
| 尺寸 | 槽位小组件可自由拖动改尺寸；行数随实际高度自适应（最多 5 行），插件无需声明尺寸 |
...
| 行数 | 单卡最多 3 行；实际渲染行数按桌面占位决定（2×1 → 1 行、2×2 → 2 行、3×2 及以上 → 3 行，脚注在 1 行时不占位） |
...
**宿主小组件显示什么**：内容**按块绑定**——桌面上可以同时放好几块 OneTHU 小组件，各显示各的
（一块日程与 DDL、一块某门课的详情、一块常用收藏夹的图标组、一块 1×1 快捷方式）。四类内容：

| 内容 | 形态 | 说明 |
|---|---|---|
| 日程与 DDL | 列表 | 今天的课、考试、作业截止；未选择内容时的缺省 |
| 一个原子占满 | 列表 | 课程 / 作业 / 洗衣机 / 教室…显示它的详情，**拉得越高行数越多**（最多 5 行）；教室给「本节空闲/占用 + 今日空闲节次」，洗衣机给「使用中 · 剩 N 分钟 + 本楼空闲台数」（捷利 / 海乐生活 / 小兰智慧三家数据源）（实时值由应用在算内容前抓取，抓不到就不写这一行） |
| 收藏夹图标组 | 图标网格 | 把收藏夹嵌到桌面：原子图标并列（最多 2 行 × 4 列），每个格子各自可点 |
| 快捷方式 | 图标 + 名称 | 一个功能页或原子的桌面快捷方式（1×1 起，像系统快捷方式） |

绑定入口三处：放到桌面时系统直接弹出选择层（AppWidget 的 configure 流程）、点桌面上还没选内容的
那块、或设置页「桌面小组件」里逐块改。内容失效（收藏夹被删、原子解析不出）自动回落成「日程与 DDL」，
不在桌面上留空白卡片。

宿主小组件有五种初始形态（1×1 快捷方式 / 2×1 窄条 / 2×2 方块 / 3×2 标准 / 4×1 长条）：选择器里可选的
形态数等于清单里的 provider 数，故形态必须多声明 provider；五者共用同一套渲染与同一份实例内容，
放置后仍可自由拖动改尺寸（行数、图标格数都按实际占位自适应）。

插件若要做「一键把本插件内容放上桌面」这类功能，可读写这些配置（需 `widget` 权限）：
...
await ctx.onethu.widget.setFallback({ kind: "today" }); // 新放置、还没选的块用哪份默认内容
```

目标不存在（收藏夹 id 不存在、原子解析不出）时返回 `false` 且不写配置。**插件不能添加或删除小组件**——
那是用户在桌面上做的事，插件只能给已经存在的那几块换内容。

**内容怎么更新**：小组件内容由宿主在前台算快照时解析（原子行每次都重新解析，因此收藏夹、
计数这类随状态变化的原子会自动跟上）。声明本身的字段（`title`）取自声明那一刻，要改就
**重新调用 `registerWidget`**（同 id 就地替换）：宿主监听注册表变化，会立刻重推快照并重画
桌面——不需要等下一次定时重算。插件停用时其小组件一并撤下（槽位让给后面的插件）。
...
| 用户可见 | 设置 → 通知 → 「插件通知」按插件列出待投递条数，可一键撤销（用户对插件的打扰有最终控制权） |
...
两点边界：①检索**不发任何校园请求**，所以只认本机已出现过的实体——没打开过的
在线服务不在结果里；②`nav.openAtom` 解析不出原子时返回 `false`，此时回话应说
「先在对应页面打开一次」，而不是宣称应用做不到。
