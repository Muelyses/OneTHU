...
| 尺寸 | 槽位小组件可自由拖动改尺寸；行数随实际高度自适应（1/2/3 行），插件无需声明尺寸 |
...
**宿主小组件显示什么**：宿主自己那块小组件的内容由**用户**决定，不与插件共用槽位。三选一：
「今天」（课程与截止，缺省）、某个收藏夹、某个收藏原子；选择入口在设置页「桌面小组件」区块与
收藏夹页编辑态工具栏（「上桌面」/ 原子长卡上的「桌」）。内容失效时（收藏夹被删、原子解析不出）
自动回落到「今天」，而不是在桌面上留一张空卡。宿主小组件提供四种初始形态
（标准 3×2 / 方块 2×2 / 窄条 2×1 / 长条 4×1），四种共用同一份内容与同一套布局，放置后仍可
自由拖动改尺寸。

插件若要做「一键把本插件内容放上桌面」这类功能，可读写这份配置（需 `widget` 权限）：

```js
const cur = await ctx.onethu.widget.getSource();      // { kind: "today" | "folder" | "atom", … }
const folders = await ctx.onethu.widget.folders();     // 用户收藏夹：[{ id, title }]
await ctx.onethu.widget.setSource({ kind: "folder", folderId: folders[0].id });
await ctx.onethu.widget.setSource(null);               // 恢复默认「今天」
```

`setSource` 只改**宿主小组件**的内容，不动插件自己的槽位；收藏夹 id 不存在或原子解析不出时返回
`false` 且不写配置。该调用会改变用户桌面上的显示内容，属用户可见的副作用，宜由用户操作触发。
